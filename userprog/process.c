...
static bool load(const char *file_name, struct intr_frame *if_) {
    ...
    char *argv_list[65]; // Allow for 64 arguments + NULL terminator
    char *fn_copy = strdup(file_name); // Make a copy of the file name to tokenize
    char *token, *save_ptr;
    int argc = 0;
    ...
    for (token = strtok_r(fn_copy, " ", &save_ptr); token != NULL; token = strtok_r(NULL, " ", &save_ptr)) {
        if (argc < 64) {
            ...
            break; // Prevent overflow if more than 64 arguments
        }
    }
    argv_list[argc] = NULL; // Null-terminate the argument list
